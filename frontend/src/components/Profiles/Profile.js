...
function Profile() {
  ...
  const fetchVideo = async () => {
    try {
      ...
      const response = await axios.get(`https://sportconnect-khom.onrender.com/api/v1/player/get-video/${id}`, {
        headers: {
        ...
        const fetchPastClubImages = async (pastClubs) => {
          return Promise.all(
            pastClubs.map(async (pastClub) => {
              const clubResponse = await axios.post('https://sportconnect-khom.onrender.com/api/v1/club/retrieve', { id: pastClub.club }, {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
              });
  ...
  const handleRemovePlayerAgent = async (playerId) => {
    try {
      ...
      toast.success(t('removePlayeragent'))
  ...
  const handleRemoveRecruiterAgent = async (recruiterId) => {
    try {
      ...
      fetchUserData(); // Refresh user data
      toast.success(t('removeCoachagent'))
    } catch (error) {
  ...
  const filteredRoleData = Object.entries(userData.player || userData.agent || userData.recruiter || userData.club || {})
    ...
    .map(([key, value]) => {
      ...
      if (key === 'experience' || key === 'expérience') {
        return [t('experience'), value.toString() + ' years']
      }
      if (key === 'dateOfBirth') {
      ...
      if (key === 'telephone') {
        return [t('telephone'), value];
      }
      if (key === 'nationality') {
        return [t('nationality'), value];
      }
      if (key === 'sports') {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'birthday' || name === 'dateOfBirth' || name === 'anniversaire') {
      setBirthday(value);
    }
    if (name === 'info') {
      setInfo(value);
    ...
    setEditedData(prevData => {
      ...
      switch (userData.role) {
        case 'player':
          return {
            ...
            player: {
              ...
              [name === 'birthday' || 'anniversaire' ? 'birthday' : name]: value,
              [name === 'pays' ? 'country' : name]: value,
              [name === 'nationalité' ? 'nationality' : name]: value,
        ...
        case 'agent':
          return {
            ...
            agent: {
              ...
              [name === 'birthday' || 'anniversaire' || name === 'dateOfBirth' ? 'dateOfBirth' : name]: value,
              [name === 'pays' ? 'country' : name]: value,
              [name === 'expérience' ? 'experience' : name]: value,
              [name === 'téléphone' ? 'telephone' : name]: value,
              [name === 'nationalité' ? 'nationality' : name]: value,
            }
          };
        case 'recruiter':
        ...
        default:
          return {
            ...prevData, [name]: value, [name === 'pays' ? 'country' : name]: value,
            [name === 'téléphone' ? 'telephone' : name]: value,
            [name === 'nationalité' ? 'nationality' : name]: value,
          };
      }
      ...
      if (userData.role === 'recruiter') {
        roleSpecificData = {
          recruiterData: {
            ...
            telephone: editedData.recruiter?.telephone,
          }
        };
      }
  ...
  const getProfileType = () => {
    if (userData.player) return t('player');
    if (userData.agent) return 'Agent';
    if (userData.club) return 'Club';
    if (userData.recruiter) return t('coach');
    return '';
  };
  const renderEditableField = (key, value) => {
    if (isEditing) {
      ...
      if (key === 'birthday' || key === 'dateOfBirth' || key === 'anniversaire') {
        return (
          <input
            type="date"
      ...
      if (key === 'experience' || key === 'expérience') {
        ...
        return (
          <input
            type="number"
            className="bg-gray-600 text-white px-2 py-1 rounded w-full"
            value={parseExperience(editedValue)}
            name={t('experience')}
            onChange={handleInputChange}
            min="0"
          />
      ...
      if (key === 'telephone' || key === 'téléphone') {
        return (
          <input
            type="number"
            className="bg-gray-600 text-white px-2 py-1 rounded w-full"
      ...
      if (key === 'typeOfTrainer') {
        return (
          <select
            ...
            className="bg-gray-700 text-white border border-gray-600 rounded-md px-3 py-2 mt-1 mb-1 text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out"
          >
            <option value="Senior">{t('coachPositions.Senior')}</option>
            <option value="Junior">{t('coachPositions.Junior')}</option>
            <option value="Goalkeeper">{t('coachPositions.Goalkeeper')}</option>
            <option value="Fitness">{t('coachPositions.Fitness')}</option>
            <option value="Assistant">{t('coachPositions.Assistant')}</option>
            <option value="Youth">{t('coachPositions.Youth')}</option>
            <option value="Technical Director">{t('coachPositions.Technical Director')}</option>
            <option value="Scout">{t('coachPositions.Scout')}</option>
          </select>
        );
      }
      if (key === 'country' || key === 'pays') {
        return (
          <select
            name={key}
            onChange={handleInputChange}
      ...
      if (key === 'nationality' || key === 'nationalité') {
        return (
          <select
            name={key}
            onChange={handleInputChange}
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white">
      ...
      >
          ...
          <div className="px-8 py-6 relative">
            ...
            <div className="mt-6">
              ...
              >
                ...
                {isEditing ? t('saveChanges') : t('editInfo')}
              </button>
            </div>
              ...
              <div>
                <div className="mt-10">
                  <h2 className="text-2xl font-semibold mb-4 text-white">
                    {userData.agent ? t('services') : userData.recruiter ? t('philosophy') : t('playerBio')}
                  </h2>
            ...
            {userData.role !== 'club' && userData.role !== 'agent' && (
              <>
                ...
                <div className="mt-10">
                  <h2 className="text-2xl font-semibold mb-4 text-white">{t('careerHistory')}</h2>
            ...
            {userData.role === 'agent' && (
              <>
                <div className="mt-10">
                  <h2 className="text-2xl font-semibold mb-4 text-white">{t('playersrep')}</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                ...
                <div className="mt-10">
                  <h2 className="text-2xl font-semibold mb-4 text-white">{t('coachrep')}</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            ...
            {userData.role === 'club' && (
              <div className="mt-10">
                <h2 className="text-3xl font-semibold mb-6 text-white">{t('clubProfile.clubDetails')}</h2>
                <div className="bg-gray-700 rounded-lg p-6 shadow-md space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="text-2xl font-semibold text-white mb-4">{t('clubProfile.generalInformation')}</h3>
                      {[t('clubProfile.clubName'), 'Address', t('clubProfile.country'), 'fullName'].map((field) => (
                        <p key={field} className="text-gray-300">
                          <span className="font-semibold">{formatKey(field === 'fullName' ? t('contactPerson') : field)}:</span>
                          {isEditing ? (
                            <input
                              type="text"
                              name={field === 'pays' ? 'country' : field==='Nom du Club' || field==='Club Name'?'clubName':field}
                              value={editedData.currentClubDetails[field] || editedData[field]}
                              onChange={handleInputChange}
                              className="ml-2 bg-gray-600 text-white px-2 py-1 rounded"
                            />
                          ) : (
                            <span className="ml-2">{field==='Nom du Club'|| field==='Club Name'?userData.currentClubDetails['clubName']:field === 'fullName'   ? userData[field] : userData.currentClubDetails[field==='Pays'|| field==='Country'?'country':field]} {console.log(userData)} </span>
                          )}
                        </p>
                      ))}

                      <p className="text-gray-300">
                        <span className="font-semibold">{t('clubProfile.contactEmail')}:</span>
                        <span className="ml-2">{userData.email}</span>
                      </p>
                    </div>
                    <div>
                      <h3 className="text-2xl font-semibold text-white mb-4">{t('clubProfile.clubInformation')}</h3>
                      {['division', 'teams'].map((field) => (
                        <p key={field} className="text-gray-300">
                          <span className="font-semibold">{formatKey(t(field))}:</span>
                          {isEditing ? (
                            <input
                              type="text"
                              name={field}
                      ...
                      <p className="text-gray-300">
                        <span className="font-semibold">{t("founded")}:</span>
                        {isEditing ? (
                          <input
                            type="date"
                            name="dateOfCreation"
                      ...
                      <p className="text-gray-300">
                        <span className="font-semibold">{t('web')}:</span>
                        {isEditing ? (
                          <input
                            type="text"
                            name="web"
                ...
                <h2 className="text-2xl font-semibold my-6 text-white">{t('clubProfile.teamRoster')}</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        ...
        {(userData.role !== 'none') && (
          <div className="mt-10">
            <h2 className="text-2xl font-semibold mb-4 text-white">{userData.player ? t('playervid') : userData.agent ? t('agentvid') : userData.club ? t('clubvid') : userData.recruiter ? t('coachvid') : null}</h2>
            <input
              type="file"
              ref={fileInputRef}
              accept="video/*"
            ...
            {video && (
              ...
              >
                ...
                <span className="font-semibold">{t('changevid')}</span>
              </button>
            )}
            {video ? (
              <div>
      ...
      <PlayerDataModal
        ...
        fetchClubPlayers={fetchClubPlayers}
        setIsPlayerModalInfoOpen={setIsPlayerModalInfoOpen}
      />
